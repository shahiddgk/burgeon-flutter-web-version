...
class _PopMenuButtonState extends State<PopMenuButton> {
  ...
  Widget build(BuildContext context) {
    ...
    bool isPhone = MediaQuery.of(context).size.width< 650;
    bool isTablet = MediaQuery.of(context).size.width >= 650 && MediaQuery.of(context).size.width < 1100;

    return PopupMenuButton(
      // add icon, by default "3 dot" icon
      // icon: Icon(Icons.book)
        padding: const EdgeInsets.only(right: 5),
        child:  Icon(Icons.more_vert,size: isPhone ? 23 : isTablet ? 27 : 30,color: AppColors.hoverColor,),
        itemBuilder: (context){
          return [
